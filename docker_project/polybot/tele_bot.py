# Users sends image from telegram bot
# Save the img in /temp/ folder
# Process the img <yolo5> and save it in <s3> & /processed
# Send the img back to telegram user
import os
from rembg import remove # or detect
from PIL import Image
from telegram import Update
from telegram.ext import ApplicationBuilder, CommandHandler, ContextTypes, MessageHandler, filters

TOKEN = '7002010210:AAHVmIDZLRTOgkUjL4pu_QxLItFcor5eSNo'

# telegram commands
async def help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=update.effective_chat.id, text='This is image object detection bot')

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=update.effective_chat.id, text='please wait while processing image')
...
async def process_image(photo_name:str):
    ...
    # pip install required modules to process img <i am using pip install rembg>
    output = remove(input) # you can replace this method with detect
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=update.effective_chat.id, text='Please wait while processing the photo...')
